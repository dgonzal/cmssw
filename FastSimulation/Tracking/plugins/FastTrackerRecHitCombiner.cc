...
FastTrackerRecHitCombiner::FastTrackerRecHitCombiner(const edm::ParameterSet& iConfig)
{
    ...
    simTracksToken = consumes<edm::SimTrackContainer>(iConfig.getParameter<edm::InputTag>("simTracks"));
    ...
    produces<FastTrackerRecHitCombinationInfoCollection>();
}
    ...
    FastTrackerRecHitCombiner::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    auto outputRefProd = iEvent.getRefBeforePut<FastTrackerRecHitCombinationCollection>();
    std::unique_ptr<FastTrackerRecHitCombinationInfoCollection> output_info(new FastTrackerRecHitCombinationInfoCollection());
	...
	if(simHitCounter == 0 || simHit.trackId() != previousSimHitTrackId){
	    ...
	    FastTrackerRecHitCombinationRef recHitCombinationRef(outputRefProd,output->size()-1);
	    SimTrackRef simTrackRef(simTracks,simHit.trackId());
	    output_info->push_back(FastTrackerRecHitCombinationInfo(recHitCombinationRef,simTrackRef));
	    previousSimHitTrackId = simHit.trackId();
	}
